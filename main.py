...
def preview_layout_embedded(canvas, pads, material, width_mm, height_mm):
    global positions
    positions.clear()
    canvas.delete("all")
    x, y = 10, 10
    ...
    for i, pad in enumerate(pads):
        ...
        for j in range(qty):
            ...
            canvas.create_text(cx, cy - radius, text=f"{pad_size:.1f}".rstrip('0').rstrip('.'), tags=tag, fill=LAYER_COLORS['engraving'])
            row.append((x, y))
...
def launch_gui():
    ...
    def on_preview():
        ...
        for material, var in material_vars.items():
            if var.get():
                preview_layout_embedded(preview_canvas, pads, material, width_mm, height_mm)
                break

# Stohrer Sax Pad SVG Generator - Full GUI and SVG Export Tool with Layout Preview
...
positions = []  # Store manually placed pad positions
...
def preview_layout(pads, material, width_mm, height_mm):
    preview = tk.Toplevel()
    preview.title("Layout Preview")
    canvas = tk.Canvas(preview, width=width_mm * 2, height=height_mm * 2, bg='white')
    canvas.pack()
    ...
    for pad in pads:
        ...
        for _ in range(qty):
            ...
            tag = f"disc_{len(positions)}_{len(row)}"
            canvas.create_oval(cx - radius, cy - radius, cx + radius, cy + radius, outline=LAYER_COLORS[material], tags=tag)
...
def launch_gui():
    ...
    def on_preview():
        ...
        for material, var in material_vars.items():
            if var.get():
                preview_layout(pads, material, width_mm, height_mm)
